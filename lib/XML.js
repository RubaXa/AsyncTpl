				...
				switch( node.name ){
					...
					case 'success':
							...
							if( node.name == 'loading' ){
								...
								if( pull.async ){
									if( node.__close ){
										val += '__ondata(\'</span><script>(function(a, b){try{b.insertBefore(a);a.parentNode.removeChild(a);}catch(er){}})(document.getElementById("%id"), document.getElementById("%name%id"));</script>'.replace(/%id/g, pull.id).replace('%name', node.name)+'\'); });'
									} else {
										val = '__this.pull(ctx,"'+pull.name+'",function('+pull.error+','+pull.as+'){'
											+ '__ondata(\'<span id="'+node.name+pull.id+'">\');'
											+ val;
									}
								}
							}
