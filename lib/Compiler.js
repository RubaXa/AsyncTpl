(function (){
	...
	function Compiler(opts){
		this._opts		= opts || { escape: true, safeMode: true, useWith: false }; 
		this._pre		= null;
		this._parser	= null;
...
//			return;
			...
			if( this._opts.useWith ){
				res = "with( ctx ){ __ondata('"+this._parser.build(res)+"'); }";
			} else {
				res = "'use strict'; __ondata('"+this._parser.build(res)+"');";
			}
//			console.log(res);
			return	new Function('ctx, __this, __ondata, __utils', res + ' __ondata();');
		},
