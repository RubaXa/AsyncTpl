(function (utils, Syntax){
	...
	// @private
		...
		_var: function (str, def){
			return ' __utils.htmlEncode((function(){try{return('+str+')}catch(e){return'+(def || '""')+'}})()) ';
//			return Syntax.safe(str);
	...
	// @public
		...
		build: function (txt, callback){
			...
			this._include(txt, function (txt){
				...
				var res = ['__reader.chunk(function(){ __buf.push(\''], match;
	...
	Compiler.compile = function (source){
		return	new Function ('__reader, ctx, __buf, __utils', source);
	};
	...
	Compiler.rules = {
		...
		'foreach\\s+(:var)\\s+as\\s+(:var)(?:\\s*=>\\s*(:var))?': function (input, key, val){
			...
			return { chunk: false, expr: '__utils.each('+this._var(input)+', function ('+val.join(',')+'){' };
//			return { chunk: false, expr: 'ctx.each('+this._var(input)+',"'+val.join('","')+'",function(ctx){' };
		},
		'/foreach': function (){ return { chunk: false, expr: '}, ctx);' } },
		...
		'pull(Sync)\\s+(:var)': function (sync, name){
			return { chunk: true, expr: 'this.pull'+sync+'('+name+',function(ctx){ ' };
		},
		'/pull': function (){ return { chunk: true, expr: '});' } },

		// closure
		'closure\\s+([^\\)]+)\\s*=>\\s*([^\\)]+)': function (a, b){
			return { chunk: true, expr: 'this.closure('+this._var(a)+',function(ctx){' };
		},
