...
//		return	new Function ('__ctx, __buf, __utils', 'with(__ctx){'+source+'}');
		return	new Function ('__reader, __ctx, __buf', source);
	};
...
//			return { chunk: false, expr: this._var(input, '[]')+'.forEach(function ('+val.join(',')+'){' };
		...
		'/foreach': function (){ return { chunk: false, expr: '}, __ctx);' } },
