	...
	Compiler.prototype = {
		...
		compile: function (input){
			var
				...
				, name
				, type
				, vars = 'undef undefined __this=this __XIF __XVAL __XFOR __XATTR __XTAG __XBIND'.split(' ')
				, before = ''
			;
			...
			while( node = input.shift() ){
				...
				if( type && type == node.VAR_NODE ){
					...
					_flush();
					var output = node.attrAny && node.attrAny('output');
					val  = ' try{__buf.'+(output == 'text' ? 'w' : 'v')+'('+node.value;
					if( output ) val += ',"'+output+'"';
					source.push(val+')}catch(e){__utils.error(e)} ');
			...
			source	= ';if(ctx.__part!==undef)__buf.off();' + source.join('') + ' __buf.end(); return __buf.toStr();';
